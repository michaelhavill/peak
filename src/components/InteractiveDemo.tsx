  ...
  return (
    <section id="agents" className="py-20 md:py-28 px-8 md:px-16 lg:px-20" style={{ borderTop: "1px solid var(--border-subtle)" }}>
      <div className="max-w-5xl mx-auto" ref={ref}>
        <motion.h2 initial={{ opacity: 0, y: 16 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-2xl md:text-3xl font-medium mb-4">Agents that actually carry your context</motion.h2>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.1 }} className="grid md:grid-cols-2 gap-5">
          <div className="rounded-xl p-6" style={{ border: "1px solid var(--border-subtle)" }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[13px] font-medium uppercase tracking-widest" style={{ color: "var(--text-secondary)" }}>Your Intent</span>
            </div>
            <p className="text-[17px] leading-relaxed" style={{ color: "var(--text-primary)" }}>&ldquo;<TypingAnimation text={DEMO_PROMPT} speed={40} />&rdquo;</p>
          </div>
          <div className="rounded-xl p-6" style={{ border: "1px solid var(--border-subtle)" }}>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: "var(--text-secondary)" }} />
              <span className="text-[13px] font-medium uppercase tracking-widest" style={{ color: "var(--text-secondary)" }}>Agent Swarm</span>
            </div>
              ...
              {DEMO_STEPS.map((step, i) => (
                <motion.div key={step.label} initial={{ opacity: 0, x: 8 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.4, delay: 0.4 + i * 0.25 }}>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[16px] font-medium" style={{ color: step.done ? "var(--text-secondary)" : "var(--text-primary)" }}>
                    ...
                    {!step.done && <span className="text-[14px] tabular-nums" style={{ color: "var(--text-secondary)" }}>{step.progress}%</span>}
                  </div>
                  {!step.done && (
