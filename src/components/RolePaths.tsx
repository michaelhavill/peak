  ...
  return (
    <section className="py-20 md:py-28 px-8 md:px-16 lg:px-20" style={{ borderTop: "1px solid var(--border-subtle)" }}>
      <div className="max-w-5xl" ref={ref}>
        <motion.h2 initial={{ opacity: 0, y: 16 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-xl md:text-2xl font-medium mb-4">Specialized paths for every role</motion.h2>
        <motion.p initial={{ opacity: 0, y: 16 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.05 }} className="text-[15px] mb-12" style={{ color: "var(--text-secondary)" }}>
          Whether you&apos;re a designer eyeing product leadership or a PM learning to vibe code — the path adapts.
        </motion.p>
        <div className="grid md:grid-cols-2 gap-5 items-start">
          ...
          <motion.div initial={{ opacity: 0, x: 16 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.15 }} className="rounded-xl p-7" style={{ border: "1px solid var(--border-subtle)" }}>
            <h3 className="text-2xl mb-3" style={{ fontFamily: "var(--font-instrument-serif)" }}>Role-Adaptive Engine</h3>
            <p className="text-[14px] leading-relaxed mb-5" style={{ color: "var(--text-secondary)" }}>Your growth path is unique. The engine analyzes your current skills, target role, and learning style to create a personalized curriculum that evolves as you do.</p>
            <div className="flex flex-wrap gap-2">
              {["Designer", "PM", "Engineer", "Leader", "Founder"].map((role) => (
